      ...
      {hasLinks && (
        <div className="absolute right-3 top-3 z-20">
          ...
          {menuOpen && (
            ...
            >
              {song.artistId && (
                <Link href={`/artist/${song.artistId}`} className="block px-3 py-2 text-sm text-white/80 transition hover:bg-white/5">
                  Go to artist
                </Link>
              )}
              {song.albumId && (
                <Link href={`/album/${song.albumId}`} className="block px-3 py-2 text-sm text-white/80 transition hover:bg-white/5">
                  Go to album
                </Link>
              )}
              <Link href={`/song/${song.id}`} className="block px-3 py-2 text-sm text-white/80 transition hover:bg-white/5">
                Song details
              </Link>
            </div>
          )}
