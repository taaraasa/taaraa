...
export default function SongRow({
  ...
  return (
    <div
      className={`group grid grid-cols-[2rem_1fr_auto] items-center gap-3 rounded-lg px-3 py-2 transition hover:bg-white/5 sm:grid-cols-[2rem_1fr_10rem_auto]`}
    >
      {/* index / play */}
      <button
        onClick={play}
      ...
      <div className="min-w-0" onClick={play} role="button">
        <p
          className={`truncate text-sm font-medium ${isActive ? "text-starlight" : "text-white"}`}
        >
          {song.title}
        </p>
        {song.artistId ? (
      ...
      <div className="hidden min-w-0 sm:block">
        {song.albumId ? (
          <Link
            ...
            className="truncate text-xs text-white/40 hover:text-white hover:underline"
          >
            {song.albumTitle}
          </Link>
        ) : null}
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 bottom-8 z-50 w-52 overflow-hidden rounded-xl border border-white/10 bg-space-800 shadow-glow">
                <p className="border-b border-white/5 px-3 py-2 text-xs font-semibold text-white/50">
                  Add to playlist
                </p>
