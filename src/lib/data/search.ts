...
export type SearchResults = {
  songs: TrackListItem[];
  artists: { id: string; name: string; verified: boolean }[];
...
export async function search(query: string): Promise<SearchResults> {
  ...
  // `mode: "insensitive"` makes matching case-insensitive on PostgreSQL.
  ...
  return {
    ...
    artists: artists.map(
      (a: { id: string; name: string; verified: boolean }) => a
    ),
    albums: albums.map(
      (al: { id: string; title: string; artist: { name: string } }) => ({
        id: al.id,
        title: al.title,
